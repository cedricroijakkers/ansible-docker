...
cat images.csv | while IFS=',' read -r pythonver ansiblever template
do
    ...
    cat "${template}" | sed -e "s/%%PLACEHOLDER_PYTHON_IMAGE%%/${pythonver}/g" | sed -e "s/%%PLACEHOLDER_ANSIBLE_VERSION%%/${ansiblever}/g" > "${ansiblever}-${pythonver}/Dockerfile"
